...
def main():
    ...
    dataset = 'ML-1M'
    path =  "/home/iec/hainn/rcm/Glocal_K/data/MovieLens_1M/"
    ...
    with tf.Session() as sess:
        ...
        for i in range(epoch_p):
            ...
            summary = Summary(value=[
                Summary.Value(tag="Pre-Training/Test RMSE", simple_value=test_rmse),
                Summary.Value(tag="Pre-Training/Train RMSE", simple_value=train_rmse),
            ])
        ...
        for i in range(epoch_f):
            ...
            if best_ndcg < test_ndcg:
                ...
                best_ndcg_ep = i+1
            summary = Summary(value=[
                Summary.Value(tag="Fine-Tuning/Test RMSE", simple_value=test_rmse),
                Summary.Value(tag="Fine-Tuning/Test MAE", simple_value=test_mae),
                Summary.Value(tag="Fine-Tuning/Test NDCG", simple_value=test_ndcg),
                Summary.Value(tag="Fine-Tuning/Train RMSE", simple_value=train_rmse),
                Summary.Value(tag="Fine-Tuning/Train MAE", simple_value=train_mae),
                Summary.Value(tag="Fine-Tuning/Train NDCG", simple_value=train_ndcg),
            ])
            train_writer.add_summary(summary, i)
            test_writer.add_summary(summary, i)
            print('.-^-._' * 12)
